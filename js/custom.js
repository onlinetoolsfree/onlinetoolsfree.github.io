...
function calculateDuration(event) {
    ...
    const resultText = `It is <strong>${durationInDays}</strong> days, or approximately <strong>${combinedText}</strong>, from the start date to the end date${includeEndDate ? ', end date included.' : '.'}`;
    ...
    // Inject results into the modal
    document.querySelector('#celebrationPopup .modal-body').innerHTML += `
        <p>${resultText}</p>
        <p>${alternativeText}</p>
    `;
